...
function navigate(view){
  ...
  document.querySelectorAll(".nav-item,.side-nav-item").forEach(b=>b.classList.toggle("active",b.dataset.view===view));
  window.scrollTo({top:0,behavior:"smooth"});
}

document.querySelectorAll(".nav-item,.side-nav-item").forEach(b=>b.addEventListener("click",()=>navigate(b.dataset.view)));
...
document.getElementById("settingsBtnDesktop")?.addEventListener("click",openSettingsDialog);
document.getElementById("settingsBtnMobile")?.addEventListener("click",openSettingsDialog);
...
if("serviceWorker" in navigator){
  window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=150", {updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{}));
}
renderAll();
