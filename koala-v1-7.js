...
function safeDebitDay(day,key){
  const n=Math.max(1,Math.min(31,Number(day)||1));
  return Math.min(n,daysInMonthKey(key));
}
function debitDateForMonth(day,key){
  return `${key}-${String(safeDebitDay(day,key)).padStart(2,"0")}`;
}
...
function setMonthTitle(){
  ...
  if(select){
    ...
    for(let offset=-12; offset<=12; offset++){
      ...
      months.push(`<option value="${key}">${monthLabel(key)}</option>`);
    }
...
function upcomingDebitsForMonth(key){
  const charges=fixedChargesForMonth(key);
  return charges.map(x=>{
    ...
    const date=debitDateForMonth(x.debitDay||1,key);
    return {charge:x,amount,date};
  }).filter(Boolean).sort((a,b)=>a.date.localeCompare(b.date));
}

function renderUpcomingDebits(){
  const el=document.getElementById("upcomingDebits");
  if(!el) return;
  let list=upcomingDebitsForMonth(selectedMonth);

  // On current month, hide debits already passed today.
  ...
  if(!list.length){
    ...
    el.textContent="Aucun prélèvement à venir pour ce mois.";
    return;
  }

  el.className="transaction-list";
  el.innerHTML=list.slice(0,6).map(({charge,amount,date})=>{
    const c=fixedCategoryInfo(charge.category);
    return `<div class="transaction-row">
      <div class="emoji">${c.emoji}</div>
      <div class="row-main">
        <strong>${escapeHtml(charge.label)}</strong>
        <small>${formatShortDate(date)} • ${c.name}</small>
      </div>
      <div class="row-value">- ${fmt(amount)}</div>
    </div>`;
  }).join("");
}
...
function renderFixedCategorySelects(){
  ...
  if(fixedSelect){
    ...
    fixedSelect.innerHTML=options;
    if(state.fixedCategories.some(c=>c.id===current)) fixedSelect.value=current;
  }

  if(filter){
    const currentFilter=filter.value;
    filter.innerHTML=`<option value="">Toutes les catégories</option>`+options;
    if(currentFilter==="" || state.fixedCategories.some(c=>c.id===currentFilter)) filter.value=currentFilter;
  }
}
...
function fixedChargeHTML(x){
  ...
  const debitDay=Number(x.debitDay)||1;
  const debitLabel=`Prélèvement le ${safeDebitDay(debitDay,monthKey())}`;
  ...
  return `<div class="transaction-row clickable" data-fixed-id="${x.id}">
    <div class="emoji">${c.emoji}</div>
    <div class="row-main"><strong>${escapeHtml(x.label)}</strong><small>${c.name} • ${debitLabel}</small>${installment}${comment}${progress}</div>
...
function renderCategorySelects(){
  ...
  if(expenseSelect){
    ...
    expenseSelect.innerHTML=state.categories.map(c=>`<option value="${c.id}">${c.emoji} ${c.name}</option>`).join("");
    if(state.categories.some(c=>c.id===current)) expenseSelect.value=current;
  }
}
...
function renderAll(){
  maybeArchiveFinishedInstallments(); ensureMonthSnapshot(selectedMonth); setMonthTitle(); renderCategorySelects(); renderHome(); renderUpcomingDebits(); renderMonthlyRecap(); renderExpenses(); renderFixed(); renderAnalysis(); renderSavings(); renderSettings();
}
...
function openFixed(id=null){
  ...
  document.getElementById("fixedDebitDay").value=1;
  if(id){
    ...
    document.getElementById("fixedDebitDay").value=x.debitDay||1;
...
document.getElementById("fixedForm").addEventListener("submit",e=>{
  ...
  const item={
    ...
    debitDay:Math.min(31,Math.max(1,Number(document.getElementById("fixedDebitDay").value)||1)),
...
function saveSettings(){
  ...
  const income=parseFloat(String(incomeInput?.value||"0").replace(",","."));
  const payDay=parseInt(payDayInput?.value||"1",10);
  state.settings.income=Number.isFinite(income)?income:0;
  state.settings.payDay=Math.min(31,Math.max(1,Number.isFinite(payDay)?payDay:1));
  syncCurrentMonthSnapshot();
  localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  renderAll();
...
document.getElementById("prevMonthBtn")?.addEventListener("click",()=>{
  ...
  const minPast=shiftMonth(monthKey(),-12);
  if(prev>=minPast){
    selectedMonth=prev;
    renderAll();
  }
...
document.getElementById("nextMonthBtn")?.addEventListener("click",()=>{
  ...
  const maxFuture=shiftMonth(monthKey(),12);
  if(next<=maxFuture){
    selectedMonth=next;
    renderAll();
  }
...
document.getElementById("menuBtn")?.addEventListener("click",()=>openDialog(menuDialog));
...
document.getElementById("saveFixedBtn")?.addEventListener("click",()=>{
  const form=document.getElementById("fixedForm");
  if(form?.requestSubmit) form.requestSubmit();
  else form?.dispatchEvent(new Event("submit",{cancelable:true,bubbles:true}));
});
document.getElementById("saveFixedCategoryBtn")?.addEventListener("click",()=>{
  const form=document.getElementById("fixedCategoryForm");
  if(form?.requestSubmit) form.requestSubmit();
  else form?.dispatchEvent(new Event("submit",{cancelable:true,bubbles:true}));
});
...
// Fermeture fiable de toutes les fenêtres, même sans enregistrer.
...
if("serviceWorker" in navigator){
  window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=170", {updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{}));
}
renderAll();
